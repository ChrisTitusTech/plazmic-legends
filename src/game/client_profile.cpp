#include "game/client_profile.h"

#include <array>

namespace plazmic {
namespace {

constexpr ClientProfile kLegendsReferenceProfile{
    .id = "legends-2026-07-29",
    .sha256 =
        "97ee793d491930ac97f91e5e26fac16d84d17ff24afcd24d5390d256e7045661",
    .machine = 0x8664U,
    .timestamp = 0x6a6a2851U,
    .optional_magic = 0x20bU,
    .image_size = 0x16c1000U,
    .character_snapshot_supported = true,
    .game_state =
        {
            .local_player_pointer_rva = 0x00f07388U,
            .world_data_pointer_rva = 0x00f07378U,
            .player_y_offset = 0x78U,
            .player_x_offset = 0x74U,
            .player_z_offset = 0x7cU,
            .player_heading_offset = 0x94U,
            .player_zone_id_offset = 0x2fcU,
            .zone_table_offset = 0x30U,
            .zone_entry_id_offset = 0x0cU,
            .zone_short_name_offset = 0x10U,
            .zone_short_name_bytes = 64U,
            .zone_id_mask = 0x7fffU,
            .maximum_zone_id = 1000U,
        },
    .spawns =
        {
            .next_offset = 0x08U,
            .previous_offset = 0x10U,
            .name_offset = 0x0b8U,
            .name_bytes = 64U,
            .type_offset = 0x139U,
            .id_offset = 0x178U,
            .level_offset = 0x6b4U,
            .y_offset = 0x78U,
            .x_offset = 0x74U,
            .z_offset = 0x7cU,
            .record_bytes = 0x6b5U,
            .maximum_count = 2048U,
        },
    .character =
        {
            .local_character_pointer_rva = 0x00f074d0U,
            .player_name_offset = 0x0b8U,
            .player_name_bytes = 64U,
            .character_zone_offset = 0x2810U,
            .character_type_descriptor_offset = 0x08U,
            .type_descriptor_displacement_offset = 0x04U,
            .stats_lookup_offset = 0x10U,
            .stats_lookup_key_offset = 0x08U,
            .stats_record_key_offset = 0x00U,
            .stats_record_value_offset = 0x08U,
            .stats_record_next_offset = 0x18U,
            .stats_maximum_records = 64U,
            .stats_current_mana_offset = 0x2764U,
            .stats_current_health_offset = 0x2770U,
            .health_adjustment_offset = 0x28U,
            .player_maximum_health_offset = 0x2b0U,
            .player_maximum_mana_offset = 0x2d0U,
            .character_base_bias = 0x08U,
            .profile_manager_offset = 0x08U,
            .profile_manager_current_type_offset = 0x08U,
            .profile_list_type_offset = 0x00U,
            .profile_list_first_profile_offset = 0x08U,
            .profile_list_next_offset = 0x18U,
            .profile_list_maximum_count = 8U,
            .inventory_container_offset = 0x20U,
            .inventory_size_offset = 0x00U,
            .inventory_data_offset = 0x08U,
            .inventory_entry_bytes = 0x10U,
            .inventory_maximum_slots = 36U,
            .item_name_pointer_offset = 0x80U,
            .item_name_bytes = 64U,
            .experience_percent_rva = 0U,
            .progression_cache_rva = 0U,
            .alternate_advancement_percent_offset = 0U,
            .alternate_advancement_points_offset = 0U,
            .unallocated_alternate_advancement_points_offset = 0U,
        },
};

constexpr ClientProfile kLegendsAugustThirdProfile{
    .id = "legends-2026-08-03",
    .sha256 =
        "f8af4e704746118f8dd94b688e585bc5c37c3d085da620136bcacad5486145ac",
    .machine = 0x8664U,
    .timestamp = 0x6a711da7U,
    .optional_magic = 0x20bU,
    .image_size = 0x16c0000U,
    .character_snapshot_supported = false,
    .game_state =
        {
            .local_player_pointer_rva = 0x00f05ff8U,
            .world_data_pointer_rva = 0x00f05fe8U,
            .player_y_offset = 0x78U,
            .player_x_offset = 0x74U,
            .player_z_offset = 0x7cU,
            .player_heading_offset = 0x94U,
            .player_zone_id_offset = 0x4a0U,
            .zone_table_offset = 0x30U,
            .zone_entry_id_offset = 0x0cU,
            .zone_short_name_offset = 0x10U,
            .zone_short_name_bytes = 64U,
            .zone_id_mask = 0x7fffU,
            .maximum_zone_id = 1000U,
        },
    .spawns =
        {
            .next_offset = 0x08U,
            .previous_offset = 0x10U,
            .name_offset = 0x0b8U,
            .name_bytes = 64U,
            .type_offset = 0x139U,
            .id_offset = 0x178U,
            .level_offset = 0x620U,
            .y_offset = 0x78U,
            .x_offset = 0x74U,
            .z_offset = 0x7cU,
            .record_bytes = 0x621U,
            .maximum_count = 2048U,
        },
    .character = {},
};

constexpr ClientProfile kLegendsAugustFourthProfile{
    .id = "legends-2026-08-04",
    .sha256 =
        "d9784a58bb03cb70177d6a494fa71bca2b13ab3c5b8b9d6c26e45bae01597e51",
    .machine = 0x8664U,
    .timestamp = 0x6a725275U,
    .optional_magic = 0x20bU,
    .image_size = 0x16bf000U,
    .character_snapshot_supported = true,
    .game_state =
        {
            .local_player_pointer_rva = 0x00f04ff8U,
            .world_data_pointer_rva = 0x00f04fe8U,
            .player_y_offset = 0x78U,
            .player_x_offset = 0x74U,
            .player_z_offset = 0x7cU,
            .player_heading_offset = 0x94U,
            .player_zone_id_offset = 0x5b0U,
            .zone_table_offset = 0x30U,
            .zone_entry_id_offset = 0x0cU,
            .zone_short_name_offset = 0x10U,
            .zone_short_name_bytes = 64U,
            .zone_id_mask = 0x7fffU,
            .maximum_zone_id = 1000U,
        },
    .spawns =
        {
            .next_offset = 0x08U,
            .previous_offset = 0x10U,
            .name_offset = 0x0b8U,
            .name_bytes = 64U,
            .type_offset = 0x139U,
            .id_offset = 0x178U,
            .level_offset = 0x64cU,
            .y_offset = 0x78U,
            .x_offset = 0x74U,
            .z_offset = 0x7cU,
            .record_bytes = 0x64dU,
            .maximum_count = 2048U,
        },
    .character =
        {
            .local_character_pointer_rva = 0x00f05140U,
            .player_name_offset = 0x0b8U,
            .player_name_bytes = 64U,
            .character_zone_offset = 0x2810U,
            .character_type_descriptor_offset = 0x08U,
            .type_descriptor_displacement_offset = 0x04U,
            .stats_lookup_offset = 0x10U,
            .stats_lookup_key_offset = 0x08U,
            .stats_record_key_offset = 0x00U,
            .stats_record_value_offset = 0x08U,
            .stats_record_next_offset = 0x18U,
            .stats_maximum_records = 64U,
            .stats_current_mana_offset = 0x2764U,
            .stats_current_health_offset = 0x2770U,
            .health_adjustment_offset = 0x28U,
            .player_maximum_health_offset = 0x2b0U,
            .player_maximum_mana_offset = 0x2e8U,
            .character_base_bias = 0x08U,
            .profile_manager_offset = 0x08U,
            .profile_manager_current_type_offset = 0x08U,
            .profile_list_type_offset = 0x00U,
            .profile_list_first_profile_offset = 0x08U,
            .profile_list_next_offset = 0x18U,
            .profile_list_maximum_count = 8U,
            .inventory_container_offset = 0x20U,
            .inventory_size_offset = 0x00U,
            .inventory_data_offset = 0x08U,
            .inventory_entry_bytes = 0x10U,
            .inventory_maximum_slots = 36U,
            .item_name_pointer_offset = 0x60U,
            .item_name_bytes = 64U,
            .experience_percent_rva = 0U,
            .progression_cache_rva = 0U,
            .alternate_advancement_percent_offset = 0U,
            .alternate_advancement_points_offset = 0U,
            .unallocated_alternate_advancement_points_offset = 0U,
        },
};

constexpr ClientProfile kLegendsAugustSixthProfile{
    .id = "legends-2026-08-06",
    .sha256 =
        "bf34438c6460acde463692fa09ea28f0d12a204e3445a9da356645fc0d475561",
    .machine = 0x8664U,
    .timestamp = 0x6a74f50aU,
    .optional_magic = 0x20bU,
    .image_size = 0x16c0000U,
    .character_snapshot_supported = true,
    .game_state =
        {
            .local_player_pointer_rva = 0x00f05ff8U,
            .world_data_pointer_rva = 0x00f05fe8U,
            .player_y_offset = 0x78U,
            .player_x_offset = 0x74U,
            .player_z_offset = 0x7cU,
            .player_heading_offset = 0x94U,
            .player_zone_id_offset = 0x5b0U,
            .zone_table_offset = 0x30U,
            .zone_entry_id_offset = 0x0cU,
            .zone_short_name_offset = 0x10U,
            .zone_short_name_bytes = 64U,
            .zone_id_mask = 0x7fffU,
            .maximum_zone_id = 1000U,
        },
    .spawns =
        {
            .next_offset = 0x08U,
            .previous_offset = 0x10U,
            .name_offset = 0x0b8U,
            .name_bytes = 64U,
            .type_offset = 0x139U,
            .id_offset = 0x178U,
            .level_offset = 0x275U,
            .y_offset = 0x78U,
            .x_offset = 0x74U,
            .z_offset = 0x7cU,
            .record_bytes = 0x276U,
            .maximum_count = 2048U,
        },
    .character =
        {
            .local_character_pointer_rva = 0x00f06140U,
            .player_name_offset = 0x0b8U,
            .player_name_bytes = 64U,
            .character_zone_offset = 0x2810U,
            .character_type_descriptor_offset = 0x08U,
            .type_descriptor_displacement_offset = 0x04U,
            .stats_lookup_offset = 0x10U,
            .stats_lookup_key_offset = 0x08U,
            .stats_record_key_offset = 0x00U,
            .stats_record_value_offset = 0x08U,
            .stats_record_next_offset = 0x18U,
            .stats_maximum_records = 64U,
            .stats_current_mana_offset = 0x2764U,
            .stats_current_health_offset = 0x2770U,
            .health_adjustment_offset = 0x28U,
            .player_maximum_health_offset = 0x2b0U,
            .player_maximum_mana_offset = 0x678U,
            .character_base_bias = 0x08U,
            .profile_manager_offset = 0x08U,
            .profile_manager_current_type_offset = 0x08U,
            .profile_list_type_offset = 0x00U,
            .profile_list_first_profile_offset = 0x08U,
            .profile_list_next_offset = 0x18U,
            .profile_list_maximum_count = 8U,
            .inventory_container_offset = 0x20U,
            .inventory_size_offset = 0x00U,
            .inventory_data_offset = 0x08U,
            .inventory_entry_bytes = 0x10U,
            .inventory_maximum_slots = 36U,
            .item_name_pointer_offset = 0x60U,
            .item_name_bytes = 64U,
            .experience_percent_rva = 0U,
            .progression_cache_rva = 0x00fa43d0U,
            .alternate_advancement_percent_offset = 0x24cU,
            .alternate_advancement_points_offset = 0x250U,
            .unallocated_alternate_advancement_points_offset = 0U,
        },
};

constexpr ClientProfile kLegendsAugustSeventeenthProfile{
    .id = "legends-2026-08-17",
    .sha256 =
        "3451069e63d5118a703a237f121a3ea7c20c973477a69fdd0d66bcdaa7d80b29",
    .machine = 0x8664U,
    .timestamp = 0x6a837d49U,
    .optional_magic = 0x20bU,
    .image_size = 0x16c2000U,
    .character_snapshot_supported = true,
    .game_state =
        {
            .local_player_pointer_rva = 0x00f08248U,
            .world_data_pointer_rva = 0x00f07d38U,
            .player_y_offset = 0x78U,
            .player_x_offset = 0x74U,
            .player_z_offset = 0x7cU,
            .player_heading_offset = 0x94U,
            .player_zone_id_offset = 0x3b8U,
            .zone_table_offset = 0x30U,
            .zone_entry_id_offset = 0x0cU,
            .zone_short_name_offset = 0x10U,
            .zone_short_name_bytes = 64U,
            .zone_id_mask = 0x7fffU,
            .maximum_zone_id = 1000U,
        },
    .spawns =
        {
            .next_offset = 0x08U,
            .previous_offset = 0x10U,
            .name_offset = 0x0b8U,
            .name_bytes = 64U,
            .type_offset = 0x139U,
            .id_offset = 0x178U,
            .level_offset = 0x391U,
            .y_offset = 0x78U,
            .x_offset = 0x74U,
            .z_offset = 0x7cU,
            .record_bytes = 0x392U,
            .maximum_count = 2048U,
        },
    .character =
        {
            .local_character_pointer_rva = 0x00f08398U,
            .player_name_offset = 0x0b8U,
            .player_name_bytes = 64U,
            .character_zone_offset = 0x2810U,
            .character_type_descriptor_offset = 0x08U,
            .type_descriptor_displacement_offset = 0x04U,
            .stats_lookup_offset = 0x10U,
            .stats_lookup_key_offset = 0x08U,
            .stats_record_key_offset = 0x00U,
            .stats_record_value_offset = 0x08U,
            .stats_record_next_offset = 0x18U,
            .stats_maximum_records = 64U,
            .stats_current_mana_offset = 0x2764U,
            .stats_current_health_offset = 0x2770U,
            .health_adjustment_offset = 0x28U,
            .player_maximum_health_offset = 0x310U,
            .player_maximum_mana_offset = 0x238U,
            .character_base_bias = 0x08U,
            .profile_manager_offset = 0x08U,
            .profile_manager_current_type_offset = 0x08U,
            .profile_list_type_offset = 0x00U,
            .profile_list_first_profile_offset = 0x08U,
            .profile_list_next_offset = 0x18U,
            .profile_list_maximum_count = 8U,
            .inventory_container_offset = 0x20U,
            .inventory_size_offset = 0x00U,
            .inventory_data_offset = 0x08U,
            .inventory_entry_bytes = 0x10U,
            .inventory_maximum_slots = 36U,
            .item_name_pointer_offset = 0x98U,
            .item_name_bytes = 64U,
            .experience_percent_rva = 0x00fa6688U,
            .progression_cache_rva = 0x00fa6440U,
            .alternate_advancement_percent_offset = 0x24cU,
            .alternate_advancement_points_offset = 0U,
            .unallocated_alternate_advancement_points_offset = 0xaa5cU,
        },
};

constexpr ClientProfile kLegendsAugustTwentyFifthProfile{
    .id = "legends-2026-08-25",
    .sha256 =
        "6807ac5c672ffee98fcc6a62a5e87d0ec6af1a323251280144a4f1461829f0d4",
    .machine = 0x8664U,
    .timestamp = 0x6a8e193aU,
    .optional_magic = 0x20bU,
    .image_size = 0x16c3000U,
    .character_snapshot_supported = true,
    .game_state =
        {
            .local_player_pointer_rva = 0x00f09248U,
            .world_data_pointer_rva = 0x00f08d38U,
            .player_y_offset = 0x78U,
            .player_x_offset = 0x74U,
            .player_z_offset = 0x7cU,
            .player_heading_offset = 0x94U,
            .player_zone_id_offset = 0x21cU,
            .zone_table_offset = 0x30U,
            .zone_entry_id_offset = 0x0cU,
            .zone_short_name_offset = 0x10U,
            .zone_short_name_bytes = 64U,
            .zone_id_mask = 0x7fffU,
            .maximum_zone_id = 1000U,
        },
    .spawns =
        {
            .next_offset = 0x08U,
            .previous_offset = 0x10U,
            .name_offset = 0x0b8U,
            .name_bytes = 64U,
            .type_offset = 0x139U,
            .id_offset = 0x178U,
            .level_offset = 0x610U,
            .y_offset = 0x78U,
            .x_offset = 0x74U,
            .z_offset = 0x7cU,
            .record_bytes = 0x611U,
            .maximum_count = 2048U,
        },
    .character =
        {
            .local_character_pointer_rva = 0x00f09398U,
            .player_name_offset = 0x0b8U,
            .player_name_bytes = 64U,
            .character_zone_offset = 0x2810U,
            .character_type_descriptor_offset = 0x08U,
            .type_descriptor_displacement_offset = 0x04U,
            .stats_lookup_offset = 0x10U,
            .stats_lookup_key_offset = 0x08U,
            .stats_record_key_offset = 0x00U,
            .stats_record_value_offset = 0x08U,
            .stats_record_next_offset = 0x18U,
            .stats_maximum_records = 64U,
            .stats_current_mana_offset = 0x2764U,
            .stats_current_health_offset = 0x2770U,
            .health_adjustment_offset = 0x28U,
            .player_maximum_health_offset = 0x420U,
            .player_maximum_mana_offset = 0x618U,
            .character_base_bias = 0x08U,
            .profile_manager_offset = 0x08U,
            .profile_manager_current_type_offset = 0x08U,
            .profile_list_type_offset = 0x00U,
            .profile_list_first_profile_offset = 0x08U,
            .profile_list_next_offset = 0x18U,
            .profile_list_maximum_count = 8U,
            .inventory_container_offset = 0x20U,
            .inventory_size_offset = 0x00U,
            .inventory_data_offset = 0x08U,
            .inventory_entry_bytes = 0x10U,
            .inventory_maximum_slots = 36U,
            .item_name_pointer_offset = 0x38U,
            .item_name_bytes = 64U,
            .experience_percent_rva = 0x00fa7688U,
            .progression_cache_rva = 0x00fa7440U,
            .alternate_advancement_percent_offset = 0x24cU,
            .alternate_advancement_points_offset = 0U,
            .unallocated_alternate_advancement_points_offset = 0xaa5cU,
        },
};

constexpr ClientProfile kLegendsSeptemberFirstProfile{
    .id = "legends-2026-09-01",
    .sha256 =
        "bc1eb76dae1a3544f37fe216a0dccc71d2323e9d11d8b612368785e80114c1c5",
    .machine = 0x8664U,
    .timestamp = 0x6a96dba9U,
    .optional_magic = 0x20bU,
    .image_size = 0x16c7000U,
    .character_snapshot_supported = true,
    .game_state =
        {
            .local_player_pointer_rva = 0x00f0d360U,
            .world_data_pointer_rva = 0x00f0ce50U,
            .player_y_offset = 0x78U,
            .player_x_offset = 0x74U,
            .player_z_offset = 0x7cU,
            .player_heading_offset = 0x94U,
            .player_zone_id_offset = 0x59cU,
            .zone_table_offset = 0x30U,
            .zone_entry_id_offset = 0x0cU,
            .zone_short_name_offset = 0x10U,
            .zone_short_name_bytes = 64U,
            .zone_id_mask = 0x7fffU,
            .maximum_zone_id = 1000U,
        },
    .spawns =
        {
            .next_offset = 0x08U,
            .previous_offset = 0x10U,
            .name_offset = 0x0b8U,
            .name_bytes = 64U,
            .type_offset = 0x139U,
            .id_offset = 0x178U,
            .level_offset = 0x4bcU,
            .y_offset = 0x78U,
            .x_offset = 0x74U,
            .z_offset = 0x7cU,
            .record_bytes = 0x4bdU,
            .maximum_count = 2048U,
        },
    .character =
        {
            .local_character_pointer_rva = 0x00f0d4b0U,
            .player_name_offset = 0x0b8U,
            .player_name_bytes = 64U,
            .character_zone_offset = 0x2810U,
            .character_type_descriptor_offset = 0x08U,
            .type_descriptor_displacement_offset = 0x04U,
            .stats_lookup_offset = 0x10U,
            .stats_lookup_key_offset = 0x08U,
            .stats_record_key_offset = 0x00U,
            .stats_record_value_offset = 0x08U,
            .stats_record_next_offset = 0x18U,
            .stats_maximum_records = 64U,
            .stats_current_mana_offset = 0x276cU,
            .stats_current_health_offset = 0x2770U,
            .stats_current_health_bytes = sizeof(std::int32_t),
            .health_adjustment_offset = 0x28U,
            .apply_health_adjustment = false,
            .player_maximum_health_offset = 0x4d0U,
            .player_maximum_mana_offset = 0x5a8U,
            .player_maximum_health_bytes = sizeof(std::int32_t),
            .player_maximum_mana_bytes = sizeof(std::int64_t),
            .character_base_bias = 0x08U,
            .profile_manager_offset = 0x08U,
            .profile_manager_current_type_offset = 0x08U,
            .profile_list_type_offset = 0x00U,
            .profile_list_first_profile_offset = 0x08U,
            .profile_list_next_offset = 0x18U,
            .profile_list_maximum_count = 8U,
            .inventory_container_offset = 0x20U,
            .inventory_size_offset = 0x00U,
            .inventory_data_offset = 0x08U,
            .inventory_entry_bytes = 0x10U,
            .inventory_maximum_slots = 36U,
            .item_name_pointer_offset = 0x68U,
            .item_name_bytes = 64U,
            .experience_percent_rva = 0x00fab528U,
            .progression_cache_rva = 0x00fab528U,
            .alternate_advancement_percent_offset = 0x04U,
            .alternate_advancement_points_offset = 0U,
            .unallocated_alternate_advancement_points_offset = 0U,
            .character_root_alternate_advancement_points_offset = 0xaa64U,
        },
};

constexpr ClientProfile kLegendsSeptemberSecondProfile{
    .id = "legends-2026-09-02",
    .sha256 =
        "f1c6ab2f07a5d08e62bb936061fd01049fa7b64ce8ddac50c57009162088a9f9",
    .machine = 0x8664U,
    .timestamp = 0x6a983abaU,
    .optional_magic = 0x20bU,
    .image_size = 0x16c6000U,
    .character_snapshot_supported = true,
    .game_state =
        {
            .local_player_pointer_rva = 0x00f0c360U,
            .world_data_pointer_rva = 0x00f0be50U,
            .player_y_offset = 0x78U,
            .player_x_offset = 0x74U,
            .player_z_offset = 0x7cU,
            .player_heading_offset = 0x94U,
            .player_zone_id_offset = 0x59cU,
            .zone_table_offset = 0x30U,
            .zone_entry_id_offset = 0x0cU,
            .zone_short_name_offset = 0x10U,
            .zone_short_name_bytes = 64U,
            .zone_id_mask = 0x7fffU,
            .maximum_zone_id = 1000U,
        },
    .spawns =
        {
            .next_offset = 0x08U,
            .previous_offset = 0x10U,
            .name_offset = 0x0b8U,
            .name_bytes = 64U,
            .type_offset = 0x139U,
            .id_offset = 0x178U,
            .level_offset = 0x4bcU,
            .y_offset = 0x78U,
            .x_offset = 0x74U,
            .z_offset = 0x7cU,
            .record_bytes = 0x4bdU,
            .maximum_count = 2048U,
        },
    .character =
        {
            .local_character_pointer_rva = 0x00f0c4b0U,
            .player_name_offset = 0x0b8U,
            .player_name_bytes = 64U,
            .character_zone_offset = 0x2810U,
            .character_type_descriptor_offset = 0x08U,
            .type_descriptor_displacement_offset = 0x04U,
            .stats_lookup_offset = 0x10U,
            .stats_lookup_key_offset = 0x08U,
            .stats_record_key_offset = 0x00U,
            .stats_record_value_offset = 0x08U,
            .stats_record_next_offset = 0x18U,
            .stats_maximum_records = 64U,
            .stats_current_mana_offset = 0x276cU,
            .stats_current_health_offset = 0x2770U,
            .stats_current_health_bytes = sizeof(std::int32_t),
            .health_adjustment_offset = 0x28U,
            .apply_health_adjustment = false,
            .player_maximum_health_offset = 0x4d0U,
            .player_maximum_mana_offset = 0x5a8U,
            .player_maximum_health_bytes = sizeof(std::int32_t),
            .player_maximum_mana_bytes = sizeof(std::int64_t),
            .character_base_bias = 0x08U,
            .profile_manager_offset = 0x08U,
            .profile_manager_current_type_offset = 0x08U,
            .profile_list_type_offset = 0x00U,
            .profile_list_first_profile_offset = 0x08U,
            .profile_list_next_offset = 0x18U,
            .profile_list_maximum_count = 8U,
            .inventory_container_offset = 0x20U,
            .inventory_size_offset = 0x00U,
            .inventory_data_offset = 0x08U,
            .inventory_entry_bytes = 0x10U,
            .inventory_maximum_slots = 36U,
            .item_name_pointer_offset = 0x68U,
            .item_name_bytes = 64U,
            .experience_percent_rva = 0x00faa538U,
            .progression_cache_rva = 0x00faa538U,
            .alternate_advancement_percent_offset = 0x04U,
            .alternate_advancement_points_offset = 0U,
            .unallocated_alternate_advancement_points_offset = 0U,
            .character_root_alternate_advancement_points_offset = 0xaa64U,
        },
};

constexpr std::array<const ClientProfile*, 8> kKnownClientProfiles{
    &kLegendsReferenceProfile,
    &kLegendsAugustThirdProfile,
    &kLegendsAugustFourthProfile,
    &kLegendsAugustSixthProfile,
    &kLegendsAugustSeventeenthProfile,
    &kLegendsAugustTwentyFifthProfile,
    &kLegendsSeptemberFirstProfile,
    &kLegendsSeptemberSecondProfile,
};

}  // namespace

const ClientProfile& legends_reference_profile() {
    return kLegendsReferenceProfile;
}

std::span<const ClientProfile* const> known_client_profiles() {
    return kKnownClientProfiles;
}

const ClientProfile* select_client_profile(std::string_view sha256) {
    for (const ClientProfile* profile : kKnownClientProfiles) {
        if (sha256 == profile->sha256) {
            return profile;
        }
    }
    return nullptr;
}

}  // namespace plazmic
